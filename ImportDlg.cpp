	...
	, m_bNoDuplicates(true)
{
	...
	DEFINE_CTRLMSG_TABLE
		CMD_CTRLMSG(IDC_BROWSE,  BN_CLICKED, &CImportDlg::OnBrowseClicked)
		CMD_CTRLMSG(IDC_REPLACE, BN_CLICKED, &CImportDlg::OnReplaceClicked)
		CMD_CTRLMSG(IDC_MERGE,   BN_CLICKED, &CImportDlg::OnMergeClicked)
	END_CTRLMSG_TABLE
}
...
bool CImportDlg::OnOk()
{
	...
	{
		AlertMsg(TXT("Please select a file to import from."));
		m_ebFileName.Focus();
		return false;
	}
...
void CImportDlg::OnBrowseClicked()
{
	...
	if (m_strFileName.Select(*this, CPath::OpenFile, szExts, TXT("CSV"), strDefFolder))
		m_ebFileName.Text(m_strFileName);
}
